...
class TriggerList extends Component {
  ...
  getOtherFields() {
      if (this.state.new_trigger_type === "Schedule"){
          return (
              <Aux>
                  <Form.Label htmlFor="trigger-schedule-time" className="pt-4">Time</Form.Label>
                  <Form.Control type="time" id="trigger-schedule-time" name="new_trigger_schedule_time" value={this.state.new_trigger_schedule_time} onChange={this.inputChangeHandler}/>
              </Aux>
          );
      }
      return null;
  ...
  triggerDeleteClickHandler(event) {
      const trigger_name = event.target.parentElement.parentElement.children[1].innerHTML
    ...
    let trigger_rows = this.state.triggers.map((trigger, index)=>{
        return (
            <tr key={'trigger_row_' + index}>
                ...
                <td>{trigger.name}</td>
                <td>{trigger.type}</td>
                <td><Button size="sm" onClick={this.triggerDeleteClickHandler}>Delete</Button></td>
            </tr>
        );
    });
        ...
        <h4>Create a New Trigger</h4>
          <Form id="create-trigger-form" onSubmit={this.formSubmitHandler}>
              <Form.Group as={Row} controlId="formTriggerName">
                  ...
                  <Form.Control type="text" id="trigger-name" name="new_trigger_name" placeholder="Trigger Name" required value={this.state.new_trigger_name} onChange={this.inputChangeHandler}/>
                  <Form.Label htmlFor="trigger-type" className="pt-4">Type</Form.Label>
                  <Form.Control as="select" id="trigger-type" name="new_trigger_type" defaultValue="Choose..." required value={this.state.new_trigger_type} onChange={this.inputChangeHandler}>
